...
```powershell
git commit -m "lp-consultation: アンケート削除・申込フォームを日程+メールのみに簡素化"
...
- **埋め込み動画**: 本編 `pCM2Dg3_iIk`
- **ファーストビューのボタン**: 「仕組みのからくりを3分で確認する（無料）」
- **アンケート**: 削除（相談時にヒアリングする方針）
- **申込フォーム**: 日程選択＋メールアドレスのみに簡素化
- **架け橋ブロック**: 「日程を選んで予約する（無料）」CTA追加
