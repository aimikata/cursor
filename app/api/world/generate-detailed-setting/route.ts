...
export async function POST(req: NextRequest) {
  try {
    ...
    const prompt = `
      ...
      **【最重要】出力は必ず日本語で行ってください。**（グローバル選択時も含む）
      ...
      **【最重要：出力言語】**
      - 出力は**すべて日本語**で記述すること（英語の混在・二言語併記は禁止）。
      - 見出しも日本語のみ（英語併記や "BILINGUAL" 表記は禁止）。
      - 固有名詞の英語表記も本文には出さない（必要なら日本語表記に統一）。
      ...
      **【指示：キャラクターの作り込み（薄さ対策）】**
      - 主人公 + ライバル（最低2名） + supportingCharacters（最低3名）を**必ず出力**。
      - 各キャラクターは「葛藤」「欲望」「弱点」「日常の行動癖」を具体化し、情報量を増やすこと。
      - 役割が被らないよう、社会階層・価値観・専門領域を差別化すること。

      **【指示：背景・世界観の厚み】**
      - 主要ロケーションは最低4つ以上。各ロケーションに「歴史背景」「構造的特徴」「物語での機能」を必ず書く。
      - 組織は最低3つ以上。各組織に「目的」「対立構造」「階層・ルール」を具体的に書く。
      - world、organizations、locations は**抽象語だけで終わらせない**（具体例を必ず添える）。

      **【指示：参照世界観レポートの再現】**
      - 入力データに含まれる「キャラクター設定」「世界観設定」「未回収リスト」「進行度」「画風タグ」を**漏れなく反映**。
      - 既に詳細がある要素は**増補のみ**（削除・要約禁止）。
      - 出力の粒度は「詳細設定資料 (Deepened Settings)」相当まで必ず掘り下げる。
