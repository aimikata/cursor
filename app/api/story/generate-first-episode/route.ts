...
export async function POST(req: NextRequest) {
  try {
    const { worldSetting, characters, storyTheme, episodeTitle, episodeNumber, volumeNumber, generationMode, masterInput, apiKey } = await req.json();
...
`.trim() : '';
    ...
    return NextResponse.json({ episode, history, usedModel });
  } catch (error: any) {
    console.error('Error generating first episode:', error);
    return NextResponse.json(
      { error: error.message || 'Failed to generate first episode' },
      { status: 500 }
    );
  }
}
